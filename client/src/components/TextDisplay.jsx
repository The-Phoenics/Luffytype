...
const TextDisplay = ({ isAudioOnRef, setIsTypingRef, reset, setReset, setStatsData }) => {
    ...
    const keyUpEvtHandler = (e) => {
        if (e.key === "Control") {
            GToggleCtrlHeldDown();
        }
    }

    const keyDownEvtHandler = (e) => {
        if (e.key === "Backspace") {
    ...
    useEffect(() => {
        ...
        const keyPressEL = document.addEventListener("keypress", keyPressEvtHandler);
        const ctrlKeyUpEL = document.addEventListener("keyup", keyUpEvtHandler);
        const keyDownEL = document.addEventListener("keydown", keyDownEvtHandler);
        ...
        <span className={`flex mt-2 mb-2 ${isWhiteSpace ? "ml-1" : ""}`}>
            {word.split("").map((letter, index) => {
                return (
                    <span
                        key={index}
                        className={`text-[clamp(1.2em,2.2vw, 4rem)] md:text-[1.7vw] font-medium min-w-[12px] font-NerdFont relative ${isWhiteSpace ? "whitespace-element" : "letter-pending"
                            }`}
                    >
                        {letter}
                        <span></span>
